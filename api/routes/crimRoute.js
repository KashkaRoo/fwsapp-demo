...
router.get("/connected", function(req, res, next) {
    res.send("crimRoute is connected");
});

// get full Xylarium_Specimen table
router.get("/", function(req , res){
	var query = "SELECT * FROM [dbo].[VW_Criminalistics_Organism_All]";
	console.log(query);
...
router.get("/getFields", function(req, res){
	...
	WHERE TABLE_NAME = 'VW_Criminalistics_Organism_All' \
...
router.get("/search/:fieldID/:searchValue", function(req, res){
	if (req.params.fieldID.toUpperCase() == "ORGANISM_ID") {
		req.params.fieldID = "o.Organism_ID";7
	}
...
router.put("/update", function(req, res) {
	...
	//console.log(req);
	var proc = "[dbo].[SP_Crim_Update_or_Add]";
...
router.get("/lastCrim", function(req, res) {
	//get the latest NFWFL Num
	var query = "SELECT TOP (1) CRIM_Num \
	FROM [" + dbConfig.database + "].[dbo].[Criminalistics] \
	order by CRIM_Num desc";
	console.log(query);
    executeQuery (query, res);
});
...
router.get("/delete/:specimenID", function(req, res){
	...
	var query = "DELETE FROM [" + dbConfig.database + "].[dbo].[Criminalistics] \
